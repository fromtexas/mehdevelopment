...
export default class Bubbles {
  ...
  init = () => {
    ...
    // bodies
    this.bodies = [];
    let totalBodies = Math.round(
      (viewportWidth * viewportHeight) / this.options.pixelsPerBody
    );
  ...
  createBody = (viewportWidth, viewportHeight) => {
    ...
    const color = this.options.colors[
      this.bodies.length % this.options.colors.length
    ];
  ...
  onScroll = currentSlide => {
    return () => {
      ...
      const delta =
        (this.lastOffset - currentSlide) * this.options.scrollVelocity;
      this.bodies.forEach(body => {
        Matter.Body.setVelocity(body, {
          x:
            body.velocity.x +
            delta * this.randomize(this.options.xVarianceRange),
          y:
            body.velocity.y +
            delta * this.randomize(this.options.yVarianceRange)
        });
      });
